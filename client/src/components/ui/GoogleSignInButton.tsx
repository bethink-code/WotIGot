const GOOGLE_CLIENT_ID = "128270666051-jatso0g5mrim3120g0rh2s323bfjn6pj.apps.googleusercontent.com";
...
export default function GoogleSignInButton({ text = "signin_with" }: GoogleSignInButtonProps) {
  ...
  const handleClick = () => {
    // Use standard OAuth2 implicit flow to get an ID token
  ...
  return (
    <button
      onClick={handleClick}
      className="w-full flex items-center justify-center gap-3 px-6 py-[14px] rounded-pill border border-[var(--border-medium)] bg-white font-dm text-[15px] text-text-dark press-scale"
    >
      <GoogleIcon />
      {label}
    </button>
